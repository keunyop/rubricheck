...
try {
  ...
  assert.equal(await page.getByText("Explore a short essay, its rubric, and an example result.", { exact: false }).count(), 0);
  const criteria = sample.getByRole("region", { name: "Evaluation Criteria", exact: true });
  assert.equal(await criteria.getByRole("article").count(), 3);
  await criteria.getByRole("heading", { name: "Evidence", exact: true }).waitFor();
  assert.equal(await page.locator("#rubric-checker").isVisible(), false);
  assert.equal(evaluations, 0);
  await sample.getByRole("region", { name: "Sample result" }).screenshot({ path: ".next/guest-sample-desktop.png", animations: "disabled" });
  ...
  const lockedCriteria = summary.getByRole("region", { name: "Locked evaluation criteria", exact: true });
  const backdrop = lockedCriteria.locator("[inert]");
  assert.equal(await backdrop.getAttribute("aria-hidden"), "true");
  assert.match(await backdrop.evaluate(element => getComputedStyle(element).filter), /blur/);
  await summary.getByText("Estimated Score Range", { exact: true }).waitFor();
  await summary.screenshot({ path: ".next/guest-summary-desktop.png", animations: "disabled" });
  pass("real summary layout and blurred criteria backdrop keep protected feedback out of the DOM");
  await page.reload();
  await summary.waitFor();
  assert.equal(evaluations, 2);
  ...
  for (const width of [768, 390, 320]) {
    ...
    const unlock = summary.getByRole("button", { name: "Sign up to see criterion feedback", exact: true });
    await unlock.scrollIntoViewIfNeeded();
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), true);
    const backgroundBox = await backdrop.boundingBox(), buttonBox = await unlock.boundingBox();
    assert.ok(buttonBox.x >= backgroundBox.x && buttonBox.x + buttonBox.width <= backgroundBox.x + backgroundBox.width);
    assert.ok(buttonBox.y >= backgroundBox.y && buttonBox.y + buttonBox.height <= backgroundBox.y + backgroundBox.height);
  }
  await page.setViewportSize({ width: 390, height: 844 });
  await lockedCriteria.screenshot({ path: ".next/guest-summary-mobile.png", animations: "disabled" });
  pass("signup overlay stays inside blurred criteria on tablet and narrow mobile screens");
